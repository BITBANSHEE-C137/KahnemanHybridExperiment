...
SG_ID=$(aws ec2 describe-security-groups \
  --filters "Name=group-name,Values=control-plane-sg" "Name=vpc-id,Values=${VPC_ID}" \
  --query "SecurityGroups[0].GroupId" \
  --output text \
  --region "${REGION}" 2>/dev/null || true)

if [[ "${SG_ID}" == "None" || -z "${SG_ID}" ]]; then
  echo "Creating security group control-plane-sg..."
  SG_ID=$(aws ec2 create-security-group \
    --group-name control-plane-sg \
  ...
  aws ec2 create-tags \
    --resources "${SG_ID}" \
    --tags Key=Name,Value=control-plane-sg Key=Project,Value=ml-lab \
    --region "${REGION}"
else
  echo "Security group control-plane-sg already exists: ${SG_ID}"
fi
...
# Install AWS CLI (not on base Ubuntu 24.04 AMI)
